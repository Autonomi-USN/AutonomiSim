...
def service_cb(request):
    ...
    rospy.Subscriber("/gazebo/set_model_state", ModelState, foreignBoatPosition)
    ...
    rospy.wait_for_message('/gazebo/set_model_state', ModelState) #!wait for foreign boat to start moving
    
    
    while True:
        #x_target, y_target'
        x_target, y_target = collision_boat_pos[0], collision_boat_pos[1]
        #Check if target reached
        #!check if the boat is on the left side of our trajectory
        """if foreignBoatOnRight():
            x_target, y_target = ultimate_target"""
        """else:
            x_target, y_target = foreign_boat_radius_position"""
...
def foreignBoatPosition(data):
    global collision_boat_pos
    collision_boat_pos = [data.pose.position.x, data.pose.position.y]
...
def updateMovingTarget(heading, velocity, location,):
    global state_target, mpc_iter, movingTargetCounter, mpc_iter, obstacle_moving_trajectory
