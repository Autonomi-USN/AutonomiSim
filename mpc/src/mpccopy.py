...
class mpc:
    def __init__(self):
        ...
        self.velocity_sub = rospy.Subscriber("/usn_drone/sensors/gps/gps/fix_velocity", Vector3Stamped, self.fixVelCallback)
        ...
        self.step_horizon = 1  # time between steps in seconds?? 0.1 Try diefferent step size.
        self.N = 5              # number of look ahead steps?? May increase to 2 sec horizon
        self.sim_time = 4000      # simulation time
        self.max_u = 2.8
        self.max_fwd_F = 28.449 #max forward thrust
        ...
        self.target_list_x = [5, 10, 15, 20, 25, 30, 35]
        self.target_list_y = [20,  0, 20,  0, 20, 0, 20]
    ...
    def fixVelCallback(self, data):
        self.u = data.vector.x
        self.v = data.vector.y
    ...
    def optimize(self, x_init, y_init, yaw_init, u_init, v_init, r_init, x_target, y_target, x_target_prev, y_target_prev):
        ...
        for k in range(0,N): #new - set the reference to track
            ...
            p[(n_states+n_controls)*(k+1)-n_states:(n_states+n_controls)*(k+1)+0] = [x_ref, y_ref, psi_ref, self.max_u, 0, 0] #check if 28*2 because two motors
            p[(n_states+n_controls)*(k+1)+0:(n_states+n_controls)*(k+1)+n_controls] = [self.max_fwd_F, self.max_fwd_F]

        p[0], p[1], p[2], p[3], p[4], p[5] = x_init, y_init, yaw_init, 0, 0, r_init
        args['p'] = p
    ...
    def run(self):
        ...
        while True:
            ...
            if self.isTargetReached(x_target, y_target):
                ...
                x_target, y_target = self.target_list_x[i], self.target_list_y[i]
                i += 1

                if i == (len(self.target_list_x) - 1):
                    break
            ...
            if cont_XP1 < 0 :
                cont_XP1 = cont_XP1/20
            else :
                cont_XP1 = cont_XP1/28.449

            if cont_XP2 < 0 :
                cont_XP2 = cont_XP2/20
            else :
                cont_XP2 = cont_XP2/28.449

            print(cont_XP1, cont_XP2)

            #Publish to ROS
            if not rospy.is_shutdown():
                self.pub_right_thrust.publish(cont_XP1)
                self.pub_left_thrust.publish(cont_XP2)
