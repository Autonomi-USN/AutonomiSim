    ...
    def camera_detection_callback(self, msg):    
        if msg.visible:                                                              # Checks to see is any objects are visible
            ...
            if self.possible_bouys_recorded < 30:                                    # Gathers points and appends them to a list of possible bouys
                self.x, self.y, z = self.get_odom_position(msg.z/100, -msg.xc/100, msg.yc/100,'usn_drone/front_camera_link',0)
                if not self.point_already_marked(self.x, self.y, z):                           # Checks that the point is not already marked 
                ...
                for i in range(30):         
                    ...
                    if len(matches_x) > 10:                                          # Filers out the values that dont match the position and puts the average value in a variable
                        ...
                        #self.publish_transform(self.x_cam, self.y_cam)
                        break
                ...
                self.possible_bouys_recorded = 0
                print('sending tf')
                self.br.sendTransform((self.x, self.y, -0.8),(0, 0, 0, 1),rospy.Time.now(),str(self.color) +"bouy_camera","usn_drone/odom")
...
#------ Calculates and publishes bouy frame from camera and rplidar data (Main function) ------#
    ...
    #             self.lidar_x, self.lidar_y, z = self.get_odom_position(lr_x, lr_y, 0,'laser',math.pi)
