...
#####################################################################################################################
#                                                                                                                   #
#   This script detects buoys and boats with the help of a tensorflow model. Draws the detections on a image        #
...
#tensorflow model.
    ...
    input_tensor    = tf.convert_to_tensor(image_np, dtype=np.uint8)
    ...
    for i in range(min(max_boxes_to_draw, boxes.shape[0])):                                                              #Loop through the detections and extract the needed information
        if scores is None or scores[i] > min_score_thresh:
            ...
            ang_x                    =  round(np.degrees(math.atan(((iteminfo[5]-960)*2*10**-4)/focallength)),6)             #Calculating the angle in x direction
            colormsg.angle           =  ang_x
            ang_y                    =  round((np.degrees(math.atan(((iteminfo[6]-540)*2*10**-4)/focallength)))*-1,6)       #Calculating the angle in y direction
            ...
            if ((iteminfo[0]=="green" or iteminfo[0]=="yellow" or iteminfo[0]=="red") and detections['detection_classes'][i]==1):
            ...
            teller=teller+1
            print("detected:",iteminfo[0],"Score:",round(scores[i]*100,2),"%",detections['detection_classes'][i])
            if (detections['detection_classes'][i]==2):
                color_publisher        =  rospy.Publisher("/Tracking/"+objecttype, VarColor , queue_size = 1)            #Publisher to publish the detection data
                color_publisher.publish(colormsg)
            if(publish_detection_image==True):
                ...
                cv2.putText(rgbimg, objecttype, (iteminfo[1],iteminfo[3]-5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (50, 205, 50), 2, cv2.LINE_AA)
    ...
    if (Yellow == False or teller==0):
      ...
      msg.visible = False
      color_publisher.publish(msg)
    if (Boat == False or teller==0):
      color_publisher = rospy.Publisher("/Tracking/boat", VarColor , queue_size = 1)
      msg = VarColor()
      msg.visible = False
      color_publisher.publish(msg)
...
detect_fn = tf.saved_model.load('/home/ros/TF-TRT/Boatmodel/saved_model') #Directory of the converted model.
